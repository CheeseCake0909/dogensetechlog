...
export async function generateMetadata({ params }: { params: { yearMonth: string } }) {
  ...
  return {
    title: `${year}年${parseInt(month)}月の記事一覧 | Dogense Tech Log`,
    description: `${year}年${parseInt(month)}月に公開された記事の一覧ページです。`,
  };
}

export default async function ArchivePage({ params }: { params: { yearMonth: string } }) {
